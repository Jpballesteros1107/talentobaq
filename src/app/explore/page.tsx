...
import InstitutionFilters, { FilterState } from '@/components/institutions/institutions-filters';
...
export default function ExplorePage() {
  ...
  useEffect(() => {
    async function fetchInstitutions() {
      ...
      if (filters.search) {
        query = query.or(
          `name.ilike.%${filters.search}%,description.ilike.%${filters.search}%,neighborhood.ilike.%${filters.search}%`
        );
      }
      ...
      if (filters.categoryIds.length > 0) {
        results = results.filter((inst) =>
          filters.categoryIds.some((cid) =>
            inst.institution_categories.some((ic) => ic.categories?.id === cid)
          )
        );
      }
      ...
      <div className="border-b border-border bg-muted/30 py-8">
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <p className="text-muted-foreground text-sm">
            Clubes deportivos y culturales verificados de Barranquilla
          </p>
        </div>
      </div>
          ...
          <aside className="hidden lg:block w-64 shrink-0">
            <div className="sticky top-24">
              <InstitutionFilters
                ...
                filters={filters}
                onChange={setFilters}
                totalResults={institutions.length}
              />
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-5 gap-4">
              <p className="text-sm text-muted-foreground hidden sm:block">
                ...
                {institutions.length === 1 ? 'institución encontrada' : 'instituciones encontradas'}
              </p>
